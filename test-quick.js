/**
 * Quick Theme Test - Chrome Desktop Only
 */
...
async function runQuickTest() {
    ...
    console.log('Quick Theme Test - Chrome Desktop');
    ...
    try {
        ...
        // Before CSS
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'quick-01-login-before.png'), fullPage: true });
        ...
        // After CSS
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'quick-02-login-after.png'), fullPage: true });
        ...
        // 3. Home page
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'quick-03-home.png'), fullPage: true });
        console.log('Home page captured');

        // 4. Navigate to a few key pages
        ...
        for (const navItem of navLinks) {
            ...
            try {
                ...
                await page.screenshot({
                    path: path.join(SCREENSHOT_DIR, 'quick-' + pageNumStr + '-' + safeName + '.png'),
                    fullPage: true
                });
                pageNum++;
            } catch (e) {
        ...
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'quick-error.png'), fullPage: true });
    } finally {
        await browser.close();
    }
}
